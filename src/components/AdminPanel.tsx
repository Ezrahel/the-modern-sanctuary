...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onExit }) => {
  ...
  const [adminSection, setAdminSection] = useState<'curation' | 'analytics'>('curation');
  ...
  return (
    <div className="min-h-screen pt-28 pb-16 px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex border-b border-surface-container-high mb-8 select-none">
          <button
            type="button"
            onClick={() => setAdminSection('curation')}
            className={`py-3 px-6 font-bold text-sm border-b-2 transition duration-150 ${adminSection === 'curation'
          ...
          <button
            ...
            className={`py-3 px-6 font-bold text-sm border-b-2 transition duration-150 ${adminSection === 'analytics'
              ? 'border-primary text-primary'
              : 'border-transparent text-secondary hover:text-charcoal'
              }`}
          >
              ...
              <div className="flex flex-wrap gap-2">
                {STATUS_TABS.map((tab) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-semibold capitalize ${statusFilter === tab.id ? 'bg-primary text-white' : 'bg-surface-container text-secondary'
                      }`}
                  >
                    {tab.label}
                  </button>
